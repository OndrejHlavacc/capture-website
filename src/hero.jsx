// Hero — large centered headline, inline product card below (ref-style).

function HeroProductCard({ lang }) {
  // A compact, legible product-card that previews the interview UI.
  // Kept SMALL so it behaves like the ref's centered screenshot.
  const L = {
  ...
  const line = lang === "cs"
    ? "Takže v pondělí stáhnu bankovní výpisy a…"
    : "So on Monday I pull the bank statements and…";
  const aiLine = lang === "cs"
    ? "Z jaké banky a kdo má přístup?"
    : "Which bank, and who has access?";
  ...
  return (
    <div
      style={{
        ...
        borderRadius: 18,
        boxShadow: "0 30px 60px -40px rgba(15,20,18,0.25), 0 2px 0 rgba(15,20,18,0.02)",
        padding: 14,
      }}
    >
      {/* card header */}
      ...
      >
        <div style={{ display: "flex", gap: 5 }}>
          <span style={{ width: 9, height: 9, borderRadius: 999, background: "#E4E1D9" }} />
          <span style={{ width: 9, height: 9, borderRadius: 999, background: "#E4E1D9" }} />
          <span style={{ width: 9, height: 9, borderRadius: 999, background: "#E4E1D9" }} />
        </div>
        <div className="mono" style={{ fontSize: 11, color: "var(--muted)", marginLeft: 6 }}>
          capture.so / sessions / close-q2
        </div>
        <div style={{ flex: 1 }} />
        <Tag tone="accent"><Dot color="var(--accent)" size={6} />{L.stage}</Tag>
      </div>

      {/* two-pane: transcript + live structure */}
      <div
        style={{
          display: "grid",
      ...
      >
        ...
        {/* right: live structured extraction */}
...
function TranscriptBubble({ who, text, tone }) {
  ...
  return (
    <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
      ...
      <div>
        <div className="mono" style={{ fontSize: 10.5, color: "var(--muted)", marginBottom: 2 }}>
          {who}
        </div>
        <div style={{ fontSize: 13.5, color: "var(--ink)", lineHeight: 1.45, maxWidth: 360 }}>
          {text}
        </div>
      </div>
    </div>
...
function MicLevel() {
  // decorative mic level bars (no animation — keeps the page calm)
  ...
  return (
    <section style={{ paddingTop: 56, paddingBottom: 40 }}>
      <Container>
        ...
        <Reveal delay={360}>
          <HeroProductCard lang={lang} />
        </Reveal>

        {/* KPI strip under hero card */}
        <Reveal delay={440}>
          <div
            style={{
              display: "grid",
          ...
          >
            {kpi.map((k, i) => (
              ...
              >
                <div className="display" style={{ fontSize: 26, fontWeight: 500, letterSpacing: "-0.02em" }}>
                  {k.n}
                </div>
                <div className="mono" style={{ fontSize: 10.5, color: "var(--muted)", marginTop: 4, letterSpacing: "0.06em", textTransform: "uppercase" }}>
                  {k.l}
                </div>
              </div>
            ))}
