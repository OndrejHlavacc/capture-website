import { Reveal, Container, Eyebrow, Tag, IconCheck } from './ui';
import { COPY } from '../copy';

const thStyle = { textAlign: "left", fontSize: 10.5, fontFamily: "'JetBrains Mono', monospace", letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--muted)", fontWeight: 500, padding: "0 8px 8px" };
const tdStyle = { padding: "10px 8px", fontSize: 13, verticalAlign: "middle" };
...
function MiniFlow() {
  return (
    <div style={{ marginTop: 8, padding: "20px 14px", border: "1px solid var(--line)", background: "var(--card)", borderRadius: 12, overflow: "hidden" }}>
      <svg viewBox="0 0 460 140" style={{ width: "100%", height: 140 }}>
        ...
        <g stroke="#B9B5A9" strokeWidth="1.2" fill="none">
          <path d="M70 40 H130" /><path d="M190 40 H250" /><path d="M310 40 H370" /><path d="M220 55 V90 H250" />
        </g>
...
export default function SlabSop({ lang }) {
  const t = COPY[lang].sop;
  const L = {
    en: {
      title: "SOP — Month-end close", version: "v3  ·  updated from capture #4", history: "History",
      tabs: ["Steps", "Flow diagram", "Versions"], rollup: "Rollup",
      r_time: "Total time", r_time_v: "2h 10m", r_cost: "Est. cost / run", r_cost_v: "€ 184", r_role: "Primary role", r_role_v: "Senior accountant",
      steps: [
      ...
      diag_title: "Flow diagram", diag_sub: "Critical steps flagged", versions: "Versions",
      v_items: [
    ...
    cs: {
      title: "SOP — Měsíční uzávěrka", version: "v3  ·  aktualizováno ze záznamu #4", history: "Historie",
      tabs: ["Kroky", "Diagram", "Verze"], rollup: "Souhrn",
      r_time: "Celkový čas", r_time_v: "2h 10m", r_cost: "Odhad nákladu / průběh", r_cost_v: "€ 184", r_role: "Hlavní role", r_role_v: "Senior účetní",
      steps: [
      ...
      diag_title: "Diagram", diag_sub: "Kritické kroky vyznačeny", versions: "Verze",
      v_items: [
  ...
  return (
    <section style={{ padding: "80px 0 56px", background: "var(--bg-soft)", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
      <Container>
        ...
        <Reveal delay={120}>
          <div style={{ border: "1px solid var(--line)", background: "var(--card)", borderRadius: 20, overflow: "hidden", boxShadow: "0 40px 80px -60px rgba(15,20,18,0.3)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px", borderBottom: "1px solid var(--line)", background: "var(--bg)" }}>
            ...
            <div style={{ display: "flex", gap: 22, padding: "8px 20px 0", borderBottom: "1px solid var(--line)" }}>
              {L.tabs.map((tab, i) => (
                <div key={tab} style={{ fontSize: 13, color: i === 0 ? "var(--ink)" : "var(--muted)", fontWeight: 500, padding: "8px 0 10px", borderBottom: i === 0 ? "2px solid var(--ink)" : "2px solid transparent" }}>{tab}</div>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr" }}>
                      ...
                      <tr key={s.n} style={{ borderTop: "1px solid var(--line)" }}>
                        ...
                        <td style={tdStyle}>
                          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                            ...
                            {s.flags.map((f) => <Tag key={f} tone={f === "approval" ? "accent" : "warn"}>{f}</Tag>)}
                          </div>
                        </td>
              ...
              <div style={{ padding: "20px 22px", background: "var(--bg)" }}>
                ...
                <div style={{ marginTop: 8 }}>
                  {L.v_items.map((v) => (
                    <div key={v.v} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 8, background: v.cur ? "var(--accent-soft)" : "var(--card)", marginBottom: 6 }}>
